jQuery(function($) {"use strict";
	...
	var Site = {
		...
		build : function() {
			...
			if ($('.fig-mobile-app').length) {
				$('.fig-mobile-app > img:first-child').addClass('animate');
				setTimeout(function() {
					$('.fig-mobile-app img:last-child').addClass('animate');
				}, 500)
			}
			...
			$(window).load(function(){
				...
				str=str.split('#')[1]
				if(str=="our-gallary?01" && $('#our-gallary').length){
					//alert(0)
					$('#our-gallary .gallery-grid .gutter-sizer + .item').find('.fancybox').trigger('click')
				}
			})
